...
session_start();
$errors = [];
$adminId = $adminName = $gender = $email = $phone = $fileName = "";
$targetFilePath = "";
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    // Save to session, cookie, and database if no errors
    if (count($errors) === 0) {
        $_SESSION['adminName'] = $adminName;
        setcookie("adminId", $adminId, time() + 3600); // 1 hour cookie
        ...
        $stmt = $conn->prepare("INSERT INTO admin_registration (adminId, adminName, gender, email, phone, filePath) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssssss", $adminId, $adminName, $gender, $email, $phone, $targetFilePath);

        if ($stmt->execute()) {
            $successMessage = "Data saved successfully to the database.";
        } else {
